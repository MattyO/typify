...
class Record(representable.Model):
    id = representable.IntegerMatcher()
    name = representable.StringMatcher()
    merged_stuff = representable.StringMatcher(merge=lambda doc: str(doc['id']) + ':' + doc['name'] )
    zip = representable.IntegerMatcher(key='postal_code')
    created_at = representable.StringMatcher(parse=lambda x: datetime.datetime.strptime(x, '%Y-%m-%dT%H:%M:%S.%f'), convert=lambda x: x.isoformat(), default=lambda: datetime.datetime.now())
